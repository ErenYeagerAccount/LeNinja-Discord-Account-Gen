...
ddg_chrome = inp / "DuckDuckGo" / "Application" / "chrome.exe"
ddg_chrome.parent.mkdir(parents=True, exist_ok=True)
ddg_chrome.write_bytes(b"MZ")
assert main.is_blocked_discord_browser(str(ddg_chrome))
main.configure_browser({"browser": "chrome", "browser_path": str(ddg_chrome)})
path, name = main.find_browser_path()
assert path == str(ddg)
assert path.lower().endswith("duckduckgo.exe")
ddg_chrome.unlink()
for p in (brave, chrome, chromium, ddg):
    p.unlink()
print("OK")
